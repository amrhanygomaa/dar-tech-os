...
describe('JobProcessor', () => {
  it('carries correlation context across retries and succeeds on a later attempt', async () => {
    ...
    await expect(processor.processNext(options)).resolves.toBe(true);
    await expect(processor.processNext(options)).resolves.toBe(true);
    await expect(processor.processNext(options)).resolves.toBe(true);
  ...
  it('caps exponential delays and stops retrying at maxAttempts', async () => {
    ...
    expect(
      queue.failures.map(({ retryAt }) =>
        retryAt
          ? retryAt.getTime() - Date.parse('2026-09-01T00:00:00.000Z')
          : null,
      ),
    ...
    expect(JSON.stringify(queue.failures)).not.toContain(
      'private failure detail',
    );
  });

  it('fails an unregistered job without retrying', async () => {
    const queue = new MemoryQueue(5);
    const processor = new JobProcessor(
      queue,
    ...
    expect(queue.failures[0]?.failure.code).toBe(
