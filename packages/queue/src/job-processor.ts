...
export type JobProcessorOptions = ClaimJobOptions;
...
export class JobProcessor {
  ...
  /** Claims and processes at most one job so the polling host stays replaceable. */
    ...
    return this.contextStore.run(
      ...
      async () => {
        ...
        try {
          ...
          const shouldRetry = normalized.retryable && job.attemptNumber < job.maxAttempts;
          const retryDelayMs = shouldRetry ? this.retryPolicy.delayMs(job.attemptNumber) : null;
          const retryAt =
            retryDelayMs === null
              ? null
              : new Date(this.clock.now().getTime() + retryDelayMs);

          await this.queue.fail({ job, failure: normalized.failure, retryAt });
