...
import { QueueDeduplicationConflictError, QueueLeaseLostError } from './errors.js';
...
function canonicalJson(value: unknown, path = 'payload'): string {
  ...
  if (value && typeof value === 'object') {
    ...
    return `{${entries
      .map(([key, nested]) => `${JSON.stringify(key)}:${canonicalJson(nested, `${path}.${key}`)}`)
      .join(',')}}`;
  }
...
function assertEnqueueInput(input: EnqueueJobInput): void {
  ...
  if (!Number.isSafeInteger(input.maxAttempts) || input.maxAttempts < 1 || input.maxAttempts > 25) {
...
function assertClaimOptions(options: ClaimJobOptions): void {
  ...
  if (
    ...
    throw new TypeError('Lease duration must be between 1 and 3600000 milliseconds');
  }
}
...
function safeFailure(failure: JobFailure): JobFailure {
  ...
  const message = failure.message
    ...
    .trim()
    .slice(0, 2_048);
  return { code, message: message || 'Job processing failed' };
}

function deduplicationHash(input: EnqueueJobInput, payloadJson: string): string {
...
export class PostgresJobQueue implements JobQueuePort {
  ...
  async enqueue(input: EnqueueJobInput): Promise<EnqueueJobResult> {
    ...
    const existing = await this.client.queueJob.findFirst({
      where: { queue: input.queue, deduplicationKey: input.deduplicationKey },
      select: { id: true, deduplicationHash: true },
    });
  ...
  async claimNext(options: ClaimJobOptions): Promise<ClaimedJob | null> {
    ...
    const rows = await this.client.$queryRaw<readonly RawClaimedJob[]>(Prisma.sql`
      WITH candidate AS (
        SELECT "id"
        FROM "queue_jobs"
        WHERE
  ...
  async fail(input: FailClaimedJobInput): Promise<void> {
    ...
    const retryAt =
      input.retryAt && input.job.attemptNumber < input.job.maxAttempts ? input.retryAt : null;
