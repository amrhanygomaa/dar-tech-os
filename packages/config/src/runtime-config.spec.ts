...
describe("runtime configuration", () => {
  ...
  it("requires bounded session lifetimes and exact deployment origins", () => {
    ...
    expect(() =>
      loadApiConfig({
        ...
        NODE_ENV: "production",
        DATABASE_URL:
          "postgresql://service:unique@db.example.invalid:5432/dartech_os",
  ...
  it("requires an explicit bounded temporary-access maximum duration", () => {
    expect(() =>
      loadApiConfig({
        ...validEnvironment,
        TEMPORARY_ACCESS_MAX_DURATION_SECONDS: undefined,
      }),
    ).toThrowError(/TEMPORARY_ACCESS_MAX_DURATION_SECONDS/);
    expect(() =>
      loadApiConfig({
        ...validEnvironment,
        TEMPORARY_ACCESS_MAX_DURATION_SECONDS: "59",
      }),
    ).toThrowError(/TEMPORARY_ACCESS_MAX_DURATION_SECONDS/);
  });
  ...
  it("rejects documented local credentials in staging without echoing the URL", () => {
    const databaseUrl =
      "postgresql://dartech:dartech@db.example.invalid:5432/dartech_os";
      ...
      expect(() =>
        loadApiConfig({
          ...
          AUTH_ALLOWED_REDIRECT_URIS:
            "https://portal.example.invalid/auth/callback",
          AUTH_LOCAL_PROVIDER_ENABLED: "true",
        }),
